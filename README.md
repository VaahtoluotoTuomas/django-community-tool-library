...
- **Core Loan System (MVP):** Secure user registration and authentication using Django's built-in services. Features automated loan period calculations based on tool categories and a robust borrowing/returning logic.
- **Accessibility First:** Built to meet WCAG 2.1 standards. Includes full keyboard navigation support (e.g., "Skip to content"), high-contrast UI themes, and dynamic ARIA attributes handled gracefully by Alpine.js.
- **Interactive UX with HTMX:** Real-time search, category filtering, and instant "inline" tool returns without full-page reloads.
- **Custom Admin Interface:** A tailored, password-protected Django administration panel allowing easy management of tools, users, and categories, complete with custom filtering and search capabilities.
- **Cloud-Native Deployment:** Fully containerized with Docker and hosted on **Azure Container Instances (ACI)**.
- **Automated CI/CD Pipeline:** GitHub Actions automatically build and push new Docker images to Azure Container Registry (ACR) upon code changes. deploy.example.sh script deploys the image and restarts the container in Azure.
- **Decoupled Storage & Database:** 
  - Media files (tool images) are served via **Azure Blob Storage**.
...
* **LinkedIn:** [Tuomas Vaahtoluoto](https://www.linkedin.com/in/tuomas-vaahtoluoto/)